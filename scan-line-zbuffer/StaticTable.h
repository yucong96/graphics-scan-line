...
private:
	void make_edge_node(const PolyNode* poly, const Eigen::Vector3f& v_top, const Eigen::Vector3f& v_bottom, EdgeNode* enode);
...
public:
	...
	void reset() {
		...
		/*for (auto edge_list : edge_table) {
		edge_list.delete_all();
		}*/
		poly_table.clear();
		//edge_table.clear();
		poly_table.resize(height);
